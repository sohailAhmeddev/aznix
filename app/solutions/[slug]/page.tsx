    ...
    <>
      <AnimatedSection className="px-6 pb-12 pt-28 sm:px-8 sm:pt-32 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-6 grid gap-6 rounded-[2rem] border border-white/10 bg-white/5 p-8 backdrop-blur lg:grid-cols-[1.15fr_0.85fr]">
            <div>
            ...
            <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/60 p-7">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-400/10 text-brand-300">
                <Icon className="h-6 w-6" />
              </div>
              <p className="mt-6 text-sm uppercase tracking-[0.25em] text-slate-500">Business impact</p>
              <p className="mt-3 text-2xl font-semibold text-white">{solution.impact}</p>
              <p className="mt-6 text-sm uppercase tracking-[0.25em] text-brand-300">Core problem</p>
              <p className="mt-3 text-base leading-7 text-slate-300">{solution.problem}</p>
            </div>
          </div>
        </div>
      </AnimatedSection>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
            ...
            <div className="mt-8 grid gap-4">
              {solution.capabilities.map((capability) => (
                <div
                  key={capability}
                  className="rounded-[1.25rem] border border-white/10 bg-slate-950/55 px-5 py-4 text-base text-slate-300"
                >
                  {capability}
                </div>
              ))}
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-5 xl:grid-cols-3">
            {relatedSolutions.map((item) => {
              ...
              return (
                <article key={item.slug} className="rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
